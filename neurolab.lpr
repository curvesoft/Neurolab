...
VAR
  ...
  VAR
    ...
    BEGIN
      ...
      FOR i := von TO min(von + 7, kan - 1) DO WITH grund[i] DO
          writeln(i : 4, schriftliste[i] : 14, extewort(multi, 2, 2) : 11, getunit : 8);
    END;
    ...
    REPEAT
      ...
      CASE upcase(readcharim('Menu Point', 'm')) OF
        'C' : BEGIN
          ...
          BEGIN
            i       := j;
            textstr := readstring('Basic SI Unit', grund[i].getunit);
        ...
        CASE fi[i] OF
          ...
          'o' : BEGIN
            ...
            filtersetz(new(offsetzg, neu(k, round(readext('Offset: Value [' +
              belegungsliste[k].getunit + ']', 0, 3, 1) / belegungsliste[k].factor))), k);
          END;
          ...
          's' : BEGIN
            ...
            filtersetz(new(spikefilterzg, neu(k, readext('Spike Filter: Max. Width '#29' [ms]', 3, 3, 1),
              readext('              Rising Gradient  '#24' [' + ableitungein.getunit + ']', 0, 3, 1),
              readext('              Falling Gradient '#25' [' + ableitungein.getunit + ']', 0, 3, 1))), k);
          END;
          ...
          'y' : BEGIN
            ...
            filtersetz(new(streckungzg, neu(k, readext('y-Resolution: max value [' +
              belegungsliste[k].getunit + ']', spannung(maxsample, k), 4, 2))), k);
          END;
          ...
          '.' : BEGIN
            ...
            filtersetz(new(digitalzg, neu(k, round(readext('Pulse counter: Threshold [' +
              belegungsliste[k].getunit + ']', 500, 3, 1) / belegungsliste[k].factor),
  ...
  PROCEDURE analogdaten;
    ...
    BEGIN
      ...
      END;
      diffn   := wordpuff;
      extpuff := kon(norm(readext('Min. amplitude [' + belegungsliste[kanal].getunit + ']',
        extspannung(rekon(minsamp, kanal), kanal), 1, 1), kanal), kanal);
      minsamp := round(mine(maxe(extpuff, minsample), maxsample));
      extpuff := kon(norm(readext('Max. amplitude [' + belegungsliste[kanal].getunit + ']',
