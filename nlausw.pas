  ...
  BEGIN
    ...
    BEGIN
      ...
      outtextxy(getmaxx, y0[j] - breite + 9, 'Maximum=' + extwort(
        (maxi[j] - hoehe[j]) * belegungsliste[k[j]].factor, 1, 2) + ' ' + belegungsliste[k[j]].getunit);
      line(max(lrand + 1, x - 20), y, min(getmaxx, x + 20), y);
      fleinheit := belegungsliste[k[j]];
      Inc(fleinheit.second);
      outtextxy(getmaxx, y0[j] - breite + 17, 'Area=' + extwort(flaeche[j] * belegungsliste[k[j]].factor, 1, 3) +
        ' ' + fleinheit.getunit);
    END;
  ...
  BEGIN
    ...
    BEGIN
      ...
      Write(plt, plpa(rdauer, fullsamplerange * (channelnumber - i)), 'CP0,-3;',
        ...
        pllb(extwort((maxi[i] - hoehe[i]) * belegungsliste[k[i]].factor, 1, 2) + ' '),
        belegungsliste[k[i]].plot, 'CP;',
        pllb(extwort(flaeche[i] * belegungsliste[k[i]].factor, 1, 3) + ' '),
        fleinheit.plot);
    END;
  END;
END;
