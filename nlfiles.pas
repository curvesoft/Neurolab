...
BEGIN
  ...
  BEGIN
    ...
    FOR j := 0 TO maxchannelsandfilters DO IF j IN kanaele.dabei THEN
      BEGIN
        ko.channels[i].Name := schriftliste[j] + ' [' + belegungsliste[j].getunit + ']';
        IF length(ko.channels[i].Name) > 10 THEN ko.channels[i].Name := schriftliste[j] + '[' + belegungsliste[j].getunit + ']';
        IF length(ko.channels[i].Name) > 10 THEN ko.channels[i].Name := schriftliste[j];
        ko.channels[i].factor1 := belegungsliste[j].factor;
