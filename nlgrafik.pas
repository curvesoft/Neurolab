...
USES  crt, graph, daff, wavpcm, tulab42, nlrahmen,
  ...
  bequem, plotter, tlfiles, SysUtils;
...
BEGIN
  ...
  BEGIN
    ...
    outtextxy(lrand + 80, getmaxy - 12, getunit);
    settextjustify(righttext, centertext);
...
BEGIN
  ...
  einheit := belegungsliste[xkanal].getunit;
  ...
  FOR i := 1 TO enn DO IF diff^[i] > 0 THEN FOR j := 1 TO breite DO
      ...
      END;
  settextjustify(lefttext, centertext);
  outtextxy(0, 14, 'Bins Width: ' + wort(enn) + ' ' + extwort(extspannung(klasse, xkanal), 3, 1) +
    ' ' + einheit);
END;
    ...
    FOR i := 1 TO channelnumber DO WITH belegungsliste[k[i]] DO
      BEGIN
        ...
        BEGIN
          ...
          settextstyle(defaultfont, horizdir, 1);
          settextjustify(lefttext, toptext);
          outtextxy(14, y0[i] - breite + 12, getunit);
          settextjustify(righttext, toptext);
          ...
          FOR i := channelnumber DOWNTO 1 DO IF NOT belegungsliste[k[i]].gepunktet THEN
            ...
            ELSE
              ...
              BEGIN
                ...
                such(dum, automn + 1, zw + dauer, tpor, dum);
                FOR tpo := tpol TO tpor DO
                  Write(plt, plpa(autom^[tpo] - zw, fullsamplerange * (channelnumber - i + 0.5) +
...
CONSTRUCTOR TChannelDataGraphic.construct(afi : BYTE; VAR kane : TChannelVolume; anf : typeextended;
  abl : SINGLE; men : menue);
CONST
  buchbr = 80;
  lupe   = 4;
VAR
  ...
  kkannr :            BYTE;
  datresult :         sample;
  extspannungresult : EXTENDED;
  getunitresult :     unitstring;
  ...
  WITH liste[aktfile] DO
    REPEAT
      ...
      IF spannstatus THEN WITH kanaele DO
        BEGIN
          //writeln('--------zei3--------');
          zwiresult := zwi(strichstelle);
          //writeln(format('zwiresult := zwi(strichstelle)= zwi(%g)=%d', [strichstelle, zwiresult]));
          kkannr := k[kannr];
          //writeln(format('kkannr := k[kannr]= k[%d]=%d', [kannr, kkannr]));
          datresult := dat(zwiresult, kkannr);
          //writeln(format('datresult := dat(zwiresult, kkannr)= dat(%d,%d)=%d', [zwiresult, kkannr, datresult]));
          extspannungresult := extspannung(datresult, kkannr);
          //writeln(format('extspannungresult := extspannung(datresult, kkannr)= extspannung(%d,%d)=%g',
            //[datresult, kkannr, extspannungresult]));
          getunitresult := belegungsliste[kkannr].getunit;
          //writeln(format('getunitresult := belegungsliste[%d].getunit', [kkannr]));
          zei3 := extwort(extspannungresult, 4, 2) + ' ' + getunitresult;

          //writeln(format('zei3=%s', [zei3]));
          //writeln('--------zei3--------');
        END
      ELSE
        zei3 := '';
...
BEGIN
  ...
  settextjustify(lefttext, centertext);
  outtextxy(0, 14, 'Bins Width: ' + wort(enn) + ' ' + extwort(extzeit(klasse), 8, 3) + ' ms');
END;
...
BEGIN
  ...
  settextjustify(lefttext, centertext);
  outtextxy(0, 14, 'Bins Width: ' + wort(enn) + ' ' + extwort(klasse, 1, 3));
END;
