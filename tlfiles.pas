...
USES  crt, dos,
  objects, bequem, daff, wavpcm, tulab42, tlfilter, SysUtils;
...
TYPE
  ...
  { angeh‰ngt, das bei jeder Suche in der Liste zu klein bzw. zu groist. Die Werte }
...
VAR
  Result : bigint64;
BEGIN
  Result := round(stelle * korr);
  //writeln(format('zwi := round(stelle * korr)=(%g * %g)=%d', [stelle, korr, Result]));
  zwi := Result;
END;
